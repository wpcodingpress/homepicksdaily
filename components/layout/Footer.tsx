    ...
    <footer style={{ background:'#0F1923', color:'rgba(255,255,255,0.85)' }}>
      ...
      <div style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'1.5rem 0' }}>
        <div className="container">
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:'1rem' }}>
            {[
              { icon: Leaf, label: 'Eco-Friendly', sub: '100% Sustainable' },
              { icon: Truck, label: 'Free Shipping', sub: 'Orders over $50' },
              { icon: Shield, label: 'Secure Checkout', sub: 'SSL Encrypted' },
              { icon: Heart, label: 'Easy Returns', sub: '30-day policy' },
              ...
              <div key={i} style={{ display:'flex', alignItems:'center', gap:'0.75rem', justifyContent:'center' }}>
                ...
                <div>
                  <div style={{ fontWeight:700, fontSize:'0.875rem', color:'white' }}>{label}</div>
      ...
      <div className="container" style={{ paddingTop:'4rem', paddingBottom:'4rem' }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:'2.5rem' }}>
          ...
          {/* Info */}
          <div>
            <h4 style={{ fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'0.9375rem', color:'white', marginBottom:'1rem' }}>Info</h4>
            <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:'0.625rem' }}>
              {infoLinks.map((label) => (
                <li key={label}>
                  <Link href={`/${label.toLowerCase().replace(/ /g,'-')}`} style={{
                    fontSize:'0.875rem', color:'rgba(255,255,255,0.55)', transition:'all 0.2s ease',
                  }}>{label}</Link>
                </li>
              ))}
          ...
          {/* Contact */}
          <div>
            <h4 style={{ fontFamily:'var(--font-heading)', fontWeight:700, fontSize:'0.9375rem', color:'white', marginBottom:'1rem' }}>Contact</h4>
            <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:'0.875rem' }}>
              {[
                { icon: Mail, label: 'hello@homepicksdaily.com', href:'mailto:hello@homepicksdaily.com' },
                { icon: Phone, label: '+1 (555) 123-4567', href:'tel:+15551234567' },
                { icon: Clock, label: 'Mon–Sat, 9am–7pm EST' },
